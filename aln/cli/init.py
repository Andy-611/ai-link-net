...
def command(
    ...
    from .host import _DEFAULT_PARENT_URL

    if not host_entry.parent_url:
        try:
            client = HostClient(base_url=host_url, timeout=5.0)
            client.host_update(HostUpdateRequest(host_name="default", parent_url=_DEFAULT_PARENT_URL))
            cli_printer.echo(f"✓ Parent host configured: {_DEFAULT_PARENT_URL}")
